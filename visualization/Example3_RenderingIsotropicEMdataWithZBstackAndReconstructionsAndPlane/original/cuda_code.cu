#include "/home/freyja/Project/Vivaldi/src/interactive_mode/helper_math.h"
...
template<typename R, typename T> GPU R point_query_3d(T* image, float3 p, int border, VIVALDI_DATA_RANGE* sdr){
	...
	for(int i=0; i < plane_cnt; i++) {
		...
		for(int j=0; j < cur_plane_cnt; j++) {
			float4 loc_plane = planes[glob_cnt];
			float3 cp_loc = cut_planes[glob_cnt];
			float loc_dist = dot(loc_plane, make_float4(x,y,z,1));
			//float plane_dist = dot(loc_plane, make_float4(cp_loc.x, cp_loc.y, cp_loc.z, 1));
			//float plane_dist = length(make_float3(x,y,z)-cp_loc);
			if(cp_loc.y - 128 < 0.0001) 
				plane_flag = true;
			if(loc_dist > 0)  
				sample_flag = true;
			glob_cnt++;
...
template<typename T> GPU line_iter orthogonal_iter(T* volume, float2 p, float step, VIVALDI_DATA_RANGE* sdr){
    ...
    float sample_depth = -99999999;
	float3 sample_depth3 = make_float3(0);
	if(wr_start.x > -99999990) {
    	sample_depth = tex2D(depthMap, p.x-wr_start.x, p.y-wr_start.y);
		//if(sample_depth > 799.5)
    		//sample_depth = -99999999;
		//else
		float sx, sy, sz;
    	sx = inv_modelview[0][0] * (p.x) + inv_modelview[0][1] * (p.y) + inv_modelview[0][2] * sample_depth + inv_modelview[0][3];
    	sy = inv_modelview[1][0] * (p.x) + inv_modelview[1][1] * (p.y) + inv_modelview[1][2] * sample_depth + inv_modelview[1][3];
    	sz = inv_modelview[2][0] * (p.x) + inv_modelview[2][1] * (p.y) + inv_modelview[2][2] * sample_depth + inv_modelview[2][3];

		sample_depth3 = make_float3(sx, sy, sz);
	}
...
//	return line_iter(make_float3(val), make_float3(val,val,val+1), 1.0);
    ...
    //if( (sample_depth - S.z) * (sample_depth - E.z) <= 0 && sample_depth > -99999990)
    	//E.z = sample_depth;
    if(sample_depth > -99999990)
		E = sample_depth3;
...
extern "C"{
__global__ void rendershortucharshortuchar3float4float4float4float3float3(RGBA* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE, short* Fish, VIVALDI_DATA_RANGE* Fish_DATA_RANGE, uchar* Mask, VIVALDI_DATA_RANGE* Mask_DATA_RANGE, short* Brain, VIVALDI_DATA_RANGE* Brain_DATA_RANGE, int x_start, int x_end, int y_start, int y_end, uchar3* colors, VIVALDI_DATA_RANGE* colors_DATA_RANGE, float4* transf, VIVALDI_DATA_RANGE* transf_DATA_RANGE, float4* zbtransf, VIVALDI_DATA_RANGE* zbtransf_DATA_RANGE, int min_val, int max_val, float4* input_col, VIVALDI_DATA_RANGE* input_col_DATA_RANGE, int num_cols, float3* plane_norm, VIVALDI_DATA_RANGE* plane_norm_DATA_RANGE, float3* plane_loc, VIVALDI_DATA_RANGE* plane_loc_DATA_RANGE){
    ...
    int plane_render_count;

    float dy;

    float dx;

    line_iter local_iter;
    ...
    col = make_float4(0);
    tmp_col = make_float4(0);

    local_iter = orthogonal_iter(Fish, x, y, 1.0124, Fish_DATA_RANGE);
    ...
    for(float3 elem = local_iter.begin(); local_iter.valid(); elem = local_iter.next()){
        ...
        if( elem.x < 2 || elem.y < 2 || elem.z < 2 || elem.x > 963-2 || elem.y > 866-2 || elem.z > 1621-2){
            continue;


        }
        val = point_query_3d<float>(Fish, elem, Fish_DATA_RANGE);

        if( val < 500){
            tmp_col = point_query_1d<float4>(transf, val, transf_DATA_RANGE);
            ...
            tur_loc = make_float3(locx+1, locy+1, locz+1);
            tul_loc = make_float3(locx+0, locy+1, locz+1);
            tdr_loc = make_float3(locx+1, locy+0, locz+1);
            tdl_loc = make_float3(locx+0, locy+0, locz+1);
            bur_loc = make_float3(locx+1, locy+1, locz+0);
            bul_loc = make_float3(locx+0, locy+1, locz+0);
            bdr_loc = make_float3(locx+1, locy+0, locz+0);
            bdl_loc = make_float3(locx+0, locy+0, locz+0);

            tur = point_query_3d<float>(Fish, tur_loc, Fish_DATA_RANGE)-999;
            tul = point_query_3d<float>(Fish, tul_loc, Fish_DATA_RANGE)-999;
            tdr = point_query_3d<float>(Fish, tdr_loc, Fish_DATA_RANGE)-999;
            tdl = point_query_3d<float>(Fish, tdl_loc, Fish_DATA_RANGE)-999;
            bur = point_query_3d<float>(Fish, bur_loc, Fish_DATA_RANGE)-999;
            bul = point_query_3d<float>(Fish, bul_loc, Fish_DATA_RANGE)-999;
            bdr = point_query_3d<float>(Fish, bdr_loc, Fish_DATA_RANGE)-999;
            bdl = point_query_3d<float>(Fish, bdl_loc, Fish_DATA_RANGE)-999;
            ...
            if( tur > 0 ){
                ...
                tmp_c = make_float3(0);
                tmp_c = point_query_1d<float3>(colors, flag, colors_DATA_RANGE);

                local_color = make_float4(tmp_c.x*local_len, tmp_c.y*local_len, tmp_c.z*local_len, 255*local_len);
        ...
        mask_elem = make_float3(elem.x + 27, elem.y , elem.z );
        mask = point_query_3d<float>(Mask, mask_elem, Mask_DATA_RANGE);
        ...
        if( mask > 0){
            ...
            float brain;
            float3 brain_elem;
            brain_elem = make_float3(elem.x / 963 * 482, elem.y / 866 * 434 ,elem.z / 1621 * 811);
            brain = linear_query_3d<float>(Brain, brain_elem, Brain_DATA_RANGE);

            b_col = make_float4(0);

            if( brain > min_val && brain < max_val){
                float4 b_col_w;
                float normalized_intensity;
                normalized_intensity = 1.0;
                b_col = make_float4(input_col[0].x*normalized_intensity, input_col[0].y*normalized_intensity, input_col[0].z*normalized_intensity, 0);
                normalized_intensity = ((brain - min_val) / (1.0*(max_val - min_val) )) *255;
                b_col_w = point_query_1d<float4>(zbtransf, normalized_intensity, zbtransf_DATA_RANGE);


                if( elem.z > 856 && elem.z < 1100){
                    b_col.w = 4.0 * b_col_w.w;
                }
                else{
                    b_col.w = 1.5 * b_col_w.w;
                }
                col = alpha_compositing(col, b_col);
        ...
        if( dist1 * prev_dist <= 0 && plane_render_count < 1){
            float4 plane_col;
            plane_col = make_float4(230, 171, 2, 120);
    ...
    if( x >= 700 && x <= 867 && y >= 900 && y <= 910){
        col = make_float4(0, 0, 0, 255);

    }
    col = background_white(col);


    rb[(x-rb_DATA_RANGE->buffer_start.x)+(y-rb_DATA_RANGE->buffer_start.y)*(rb_DATA_RANGE->buffer_end.x-rb_DATA_RANGE->buffer_start.x)] = RGBA(col);
    return;
...
__global__ void rendershortucharshortuchar3float4float4float4float3float3_checker(RGBA* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE, short* Fish, VIVALDI_DATA_RANGE* Fish_DATA_RANGE, uchar* Mask, VIVALDI_DATA_RANGE* Mask_DATA_RANGE, short* Brain, VIVALDI_DATA_RANGE* Brain_DATA_RANGE, int x_start, int x_end, int y_start, int y_end, uchar3* colors, VIVALDI_DATA_RANGE* colors_DATA_RANGE, float4* transf, VIVALDI_DATA_RANGE* transf_DATA_RANGE, float4* zbtransf, VIVALDI_DATA_RANGE* zbtransf_DATA_RANGE, int min_val, int max_val, float4* input_col, VIVALDI_DATA_RANGE* input_col_DATA_RANGE, int num_cols, float3* plane_norm, VIVALDI_DATA_RANGE* plane_norm_DATA_RANGE, float3* plane_loc, VIVALDI_DATA_RANGE* plane_loc_DATA_RANGE){
