def main():
    ...
    Rot_Start = VIVALDI_WRITE('Rot_Start', 90 if len(sys.argv) < 3 else int(sys.argv[2]))
    Rot_End   = VIVALDI_WRITE('Rot_End', 91 if len(sys.argv) < 4 else int(sys.argv[3]))
    ...
    for elem in range(Rot_Start,Rot_End):
        ...
        result = VIVALDI_WRITE('result', run_function(return_name='result', func_name='render', args=[volume, volume_mask, volume_brain, 'x', 'y', transf, transf1, start, end, input_col, Plane_normal, Plane_loc, ], arg_names=['volume', 'volume_mask', 'volume_brain', 'x', 'y', 'transf', 'transf1', 'start', 'end', 'input_col', 'Plane_normal', 'Plane_loc'], execid=[], work_range={'y':(-800,1000),'x':(-1024,1024)}))

        result = VIVALDI_WRITE('result', VIVALDI_GATHER(result))
        save_image(result, "%srotate/%s%03d.png"%(today,today,elem))

    import os
    os.system("send_telegram Done")
