...
extern "C"{
__global__ void renderucharucharucharuchar4(RGBA* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE, uchar* mask, VIVALDI_DATA_RANGE* mask_DATA_RANGE, uchar* img, VIVALDI_DATA_RANGE* img_DATA_RANGE, uchar* volume, VIVALDI_DATA_RANGE* volume_DATA_RANGE, int x_start, int x_end, int y_start, int y_end, uchar4* transf, VIVALDI_DATA_RANGE* transf_DATA_RANGE){
    ...
    norm = make_float3(0.6901, -0.7236, -0.113);
    loc  = make_float3(881/2.0, 1823/2.0, 10135/12.5/2.0);

    d0 = 0.0;
    d0 = -norm.x*loc.x - norm.y*loc.y - norm.z * loc.z;

    norm1 = make_float3(0.7502, 0.6612, -0.0003/12.5);
    loc1  = make_float3(879/2.0, 1691/2.0, 10056/12.5/2.0);
    d1 = 0.0;
    ...
    lim_x_ye  = make_float2(546/2.0, 1216/2.0);
    lim_y_ye  = make_float2(1356/2.0, 2026/2.0);

    lim_x_ma  = make_float2(606/2.0, 1501/2.0);
    lim_y_ma  = make_float2(1356/2.0, 2096/2.0);
    ...
    for(float3 elem = line_iter.begin(); line_iter.valid(); elem = line_iter.next()){
        ...
        int flag_ye;
        float val_ma;
        val_ma = point_query_3d<float>(mask, elem.x/8.0, elem.y/8.0, elem.z*2, mask_DATA_RANGE);
        ...
        if( elem.x > lim_x_ye.x && elem.y > lim_y_ye.x && elem.x < lim_x_ye.y && elem.y < lim_y_ye.y){
            flag_ye = 1;

        }
        if( elem.x > lim_x_ma.x && elem.y > lim_y_ma.x && elem.x < lim_x_ma.y && elem.y < lim_y_ma.y){
            flag_ma = 1;

        }
        if( elem.z < step*5  && elem.z > 0.0 && outer_flag < 1.0){
            float img_val;
            img_val = linear_query_2d<float>(img, elem.x*2.0, elem.y*2.0, img_DATA_RANGE);
            if( img_val > 0.0){
                tmp_col = make_float4(11,119,51,127.5);
        ...
        if( dd0 * prev_dd_ma <= 0.0 && flag_ma > 0 && magenta_flag < 10.0){
            tmp_col = make_float4(231, 41, 138, 127.5);
            color = alpha_compositing(color, tmp_col);
            magenta_flag += 1.0;
        }
        prev_dd_ma = dd0;
        ...
        if( dd0 * prev_dd_ye <= 0.0 && flag_ye > 0 && yellow_flag < 10.0){
            tmp_col = make_float4(230, 171, 2, 127.5);
            color = alpha_compositing(color, tmp_col);
            yellow_flag += 1.0;
        }
        else{
            if( val_ma > 100){
                float val;
                val = point_query_3d<float>(volume, elem, volume_DATA_RANGE);
                tmp_col = point_query_1d<float4>(transf, val, transf_DATA_RANGE);
                if( (tmp_col.x + tmp_col.y + tmp_col.z) > 0){
                    if( val < 50){
                        val = val * 10;
                    }
                    tmp_col = make_float4(tmp_col.x, tmp_col.y, tmp_col.z, tmp_col.w / 2.5);
                }
                else{
                    tmp_col = make_float4(val,val,val,val / 3000);
    ...
    rb[(x-rb_DATA_RANGE->buffer_start.x)+(y-rb_DATA_RANGE->buffer_start.y)*(rb_DATA_RANGE->buffer_end.x-rb_DATA_RANGE->buffer_start.x)] = RGBA(color);
    return;

}

}extern "C"{
__global__ void renderucharucharucharuchar4_checker(RGBA* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE, uchar* mask, VIVALDI_DATA_RANGE* mask_DATA_RANGE, uchar* img, VIVALDI_DATA_RANGE* img_DATA_RANGE, uchar* volume, VIVALDI_DATA_RANGE* volume_DATA_RANGE, int x_start, int x_end, int y_start, int y_end, uchar4* transf, VIVALDI_DATA_RANGE* transf_DATA_RANGE){
