...
for row in rows:
    for col in cols:
        ...
        # reduce any 255 values to 254 because 255 will be the new transparency
        tile_data = numpy.array(tile)
        tile_data = numpy.clip(tile_data, 0, 254)
        tile_clip = Image.fromarray(tile_data)

        # for y in xrange(tile.size[1]):
        #     for x in xrange(tile.size[0]):
        #         if tile_data[x, y][:3] == (255, 255, 255):
        #             tile_data[x, y] = list(tile_data[x, y])
        #             tile_data[x, y][:3] = [254, 254, 254]
        #             tile_data[x, y] = tuple(tile_data[x, y])
        #             print "WARNING: decreased intensity of white pixel (" + \
        #                   str(x) + "," + str(y) + ")"

        # copy non-alpha regions of tile into new RGB tile with background 255
        new_tile = Image.new('RGB', tile.size, (255, 255, 255))
        new_tile.paste(tile_clip, mask=tile.split()[-1]) # -1 is the alpha channel

        # convert the RGB tile into a luminance tile
        new_tile = new_tile.convert('L')

        # set the tRNS flag in the image info 
        new_tile_info = new_tile.info
        new_tile_info['transparency'] = 255

        # save re-composed sect_image
        new_tile.save(os.path.join(save_path, str(row) + "_" + str(col) + ext), **new_tile_info)
